...
Deno.test("unwrap throws for err results", () => {
  ...
  try {
    ...
    const error = e as ResultError<string>;
    assertEquals(error instanceof ResultError, true);
  }
});
...
Deno.test("toOption returns null for err results", () => {
  const result: Result<number, string> = { error: "error" };
  assertEquals(toOption(result), null);
});
